...
var globl = require('../clientStateHandler');
var BrowserController = require('../browserController');
var bc = new BrowserController();
...
WebdriverMarionetteInterface.prototype.session_sessionId_location = {
  register: function (server, client) {
    server.route({
        method: 'POST',
  ...
  post: function (request, reply) {
    ...
    var remoteFn = function (location) {
      if (!window.__DALEK) window.__DALEK = {};
      window.__DALEK.location = location;
      return true;
    };
    
    // TODO: Check if this hack works
    globl.getClient().executeScript(remoteFn, [location], function (err, done) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));  
      }
      reply(webdriveryfiResponse(null, done, globl.getClient().actor));
  ...
  get: function (request, reply) {
    ...
    var remoteFn = function () {
      return window.__DALEK && window.__DALEK.location ? window.__DALEK.location : null;
    };
    
    // TODO: Does only worked if mocked before
    globl.getClient().executeScript(remoteFn, [], function (err, location) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }
      ...
      if (!location) {
        reply(webdriveryfiResponse('err', 'No location set!', globl.getClient().actor));  
      }
...
WebdriverMarionetteInterface.prototype.session_sessionId_local_storage = {
  register: function (server, client) {
    server.route({
        method: 'POST',
  ...
  post: function (request, reply) {
    ...
    var remoteFn = function (key, value) {
      return window.localStorage[key] = value;
    };
    
    // TODO: Check if this hack works
    globl.getClient().executeScript(remoteFn, [request.payload.key, request.payload.value], function (err, done) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));  
      }
      reply(webdriveryfiResponse(null, done, globl.getClient().actor));
  ...
  get: function (request, reply) {
    ...
    // TODO: Does only worked if mocked before
    globl.getClient().executeScript(remoteFn, [], function (err, localstorage) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }
      
      reply(webdriveryfiResponse(null, localstorage, globl.getClient().actor));
    });
  },
  ...
  delete: function (request, reply) {
    ...
    // TODO: Does only worked if mocked before
    globl.getClient().executeScript(remoteFn, [], function (err, done) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }
...
WebdriverMarionetteInterface.prototype.session_sessionId_local_storage_key_key = {
  register: function (server, client) {
    server.route({
        method: 'GET',
  ...
  get: function (request, reply) {
    ...
    var remoteFn = function (key) {
      return window.localStorage[key];
    };
    
    // TODO: Does only worked if mocked before
    globl.getClient().executeScript(remoteFn, [request.params.key], function (err, localstorage) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }
  ...
  delete: function (request, reply) {
    ...
    // TODO: Does only worked if mocked before
    globl.getClient().executeScript(remoteFn, [request.params.key], function (err, done) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }
...
WebdriverMarionetteInterface.prototype.session_sessionId_local_storage_key_key = {
  ...
  get: function (request, reply) {
    ...
    // TODO: Does only worked if mocked before
    globl.getClient().executeScript(remoteFn, [request.params.key], function (err, size) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }
...
WebdriverMarionetteInterface.prototype.session_sessionId_session_storage = {
  register: function (server, client) {
    server.route({
        method: 'POST',
...
WebdriverMarionetteInterface.prototype.session_sessionId_session_storage_key_key = {
  register: function (server, client) {
    server.route({
        method: 'GET',
...
WebdriverMarionetteInterface.prototype.session_sessionId_log = {
  register: function (server, client) {
    server.route({
        method: 'POST',
  ...
  get: function (request, reply) {
    ...
    if (!request.payload.type) {
      // set proper statuscode / error message
      reply();
    }
    
    // TODO: Does only worked if mocked before
    globl.getLogs(function (err, logs) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }
      
      reply(webdriveryfiResponse(null, logs, globl.getClient().actor));
    });
  }
};
...
WebdriverMarionetteInterface.prototype.session_sessionId_log_types = {
  register: function (server, client) {
    server.route({
        method: 'GET',
  ...
  get: function (request, reply) {
    ...
    // TODO: Does only worked if mocked before
    reply(webdriveryfiResponse(null, ['std'], globl.getClient().actor));
  }
};
...
WebdriverMarionetteInterface.prototype.session_sessionId_application_cache_status = {
  register: function (server, client) {
    server.route({
        method: 'GET',
  ...
  get: function (request, reply) {
    ...
    // TODO: Does only worked if mocked before
    globl.getClient().executeScript(remoteFn, [request.params.key], function (err, status) {
      if (err) {
        reply(webdriveryfiResponse(err, err, globl.getClient().actor));
        return; 
      }

      reply(webdriveryfiResponse(null, status, globl.getClient().actor));
    });
  }
};
